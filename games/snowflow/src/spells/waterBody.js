/**
 * The bent-water renderer — one mesh, one material, one draw, sixteen strands.
 *
...
/** Must match `array<vec4f, 16>` in `water.vertex.wgsl`; nine spells need 15 slots. */
export const STRAND_MAX = 16;
...
export class WaterBody {
    ...
    /** Capsule tests against the same CPU spine that renders each water spell. */
    intersectsSphere(x, y, z, radius, ignoredStrand = -1) {
        const d = this._texData;
        for (let s = 0; s < STRAND_MAX; s++) {
            if (s === ignoredStrand || !this._used[s] || this._params[s * 4 + 2] < 0.08) continue;
            const count = this._params[s * 4 + 3];
            const base = s * 3 * STRAND_COLS * 4;
            for (let c = 1; c < count; c++) {
                const a = base + (c - 1) * 4, b = a + 4;
                const thickness = Math.max(d[a + 3], d[b + 3]);
                if (thickness < 0.015) continue;
                const dx = d[b] - d[a], dy = d[b + 1] - d[a + 1], dz = d[b + 2] - d[a + 2];
                const length2 = dx * dx + dy * dy + dz * dz;
                const t = Math.max(0, Math.min(1,
                    ((x - d[a]) * dx + (y - d[a + 1]) * dy + (z - d[a + 2]) * dz) / (length2 || 1)));
                const ex = x - d[a] - dx * t, ey = y - d[a + 1] - dy * t, ez = z - d[a + 2] - dz * t;
                if (ex * ex + ey * ey + ez * ez <= (radius + thickness) ** 2) return true;
            }
        }
        return false;
    }
