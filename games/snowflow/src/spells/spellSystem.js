...
import { WaterShield } from "./waterShield.js";
import { IceLances } from "./iceLances.js";
import { TidalWave } from "./tidalWave.js";
import { FrostComet } from "./frostComet.js";
...
export class SpellSystem {
    ...
    constructor(scene, sky, shadows, terrain, controller, figure, rig, spray) {
        ...
        this.waterShield = new WaterShield(this.ctx);
        this.iceLances = new IceLances(this.ctx);
        this.tidalWave = new TidalWave(this.ctx);
        this.frostComet = new FrostComet(this.ctx);

        this.spells = [this.sweep, this.ribbon, this.bloom, this.crystallize, this.vortex,
            this.waterShield, this.iceLances, this.tidalWave, this.frostComet];
    ...
    update(dt, cameraPos) {
        ...
        const casting =
            this.ribbon.active || this.iceLances.active ||
            (this.frostComet.active && !this.frostComet.impacted) ||
    ...
    hitsMonster(x, y, z) {
        if (S.showSpells === false) return 0;
        const crystal = this.crystallize, comet = this.frostComet, lances = this.iceLances;
        if (crystal.active && crystal.t > 0.25 && Math.abs(y - crystal.y) < 3 &&
            Math.hypot(x - crystal.x, z - crystal.z) < 2.9) return 2;
        if (comet.active && comet.impacted && Math.abs(y - comet.y) < 4 &&
            Math.hypot(x - comet.x, z - comet.z) < 4.8) return 2;
        if (lances.active) {
            for (let i = 0; i < 3; i++) {
                const o = i * 3;
                if (lances.hit[i] && Math.abs(y - lances.targets[o + 1]) < 3 &&
                    Math.hypot(x - lances.targets[o], z - lances.targets[o + 2]) < 1.8) return 2;
            }
        }
        const preview = comet.active && !comet.impacted ? comet.strands[1] : -1;
        return this.water.intersectsSphere(x, y + 0.9, z, 0.95, preview) ? 1 : 0;
    }
     ...
     */
    cast(key) {
        if (S.showSpells === false || !Number.isInteger(key) || key < 1 || key > 9) return;
        const ctx = this.ctx;
        const rig = ctx.rig;
        ...
        if (key === 6) this.waterShield.trigger();
    }
