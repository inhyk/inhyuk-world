...
fn main(input: FragmentInputs) -> FragmentOutputs {
    ...
    if (mu > discCos) {
        ...
        col += uniforms.sunColor * uniforms.sunIntensity * 42.0 * limb * (1.0 - uniforms.nightAmount);
    }
    let aureole = pow(max(0.0, mu), 1400.0) * 5.5 + pow(max(0.0, mu), 64.0) * 0.28;
    col += uniforms.sunColor * uniforms.sunIntensity * aureole * 0.5 * (1.0 - uniforms.nightAmount);
    ...
    // Stable moonlight grading avoids rebaking/readback of the atmosphere every
    // frame. Mountains stay in the same atmosphere; stars never paint over them.
    col *= mix(vec3f(1.0), vec3f(0.25, 0.38, 0.65), uniforms.nightAmount);
    if (!mountainHit && dir.y > 0.025 && uniforms.nightAmount > 0.001) {
        let cell = floor(uv * vec2f(1100.0, 550.0));
        let seed = fract(sin(dot(cell, vec2f(127.1, 311.7))) * 43758.5453);
        let point = fract(uv * vec2f(1100.0, 550.0)) - vec2f(0.5);
        let star = (1.0 - smoothstep(0.035, 0.21, length(point))) * step(0.991, seed);
        let twinkle = 0.8 + 0.2 * sin(uniforms.time * 1.4 + seed * 500.0);
        col += vec3f(3.5, 5.0, 7.0) * star * twinkle * uniforms.nightAmount * smoothstep(0.03, 0.2, dir.y);

        let moonDir = normalize(vec3f(0.58, 0.35, -0.74));
        let moonDistance = length(dir - moonDir);
        let moon = 1.0 - smoothstep(0.015, 0.016, moonDistance);
        let craters = 0.8 + 0.2 * sin(dir.x * 900.0) * cos(dir.z * 750.0);
        let halo = exp(-moonDistance * 40.0) * 0.5;
        col += vec3f(18.0, 23.0, 30.0) * (moon * craters + halo) * uniforms.nightAmount;
    }
