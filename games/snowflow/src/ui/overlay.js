...
#ov {
  ...
  width: min(336px, 100vw); display: none;
...
export class Overlay {
    ...
    constructor(refs) {
        ...
        hdr.className = "hdr";
        hdr.innerHTML = '<b>화질 · 설정</b><button type="button" class="quiet-button" aria-label="설정 닫기">닫기 ×</button>';
        hdr.querySelector("button").onclick = () => window.dispatchEvent(
            new KeyboardEvent("keydown", { code: "F1", bubbles: true })
        );
        el.appendChild(hdr);
        ...
        ph.textContent = "화질 · 느리면 균형 모드를 선택하세요";
        el.appendChild(ph);
        ...
        for (const name of ["ultra", "high", "balanced"]) {
            ...
            b.textContent = { ultra: "최고", high: "높음", balanced: "균형" }[name];
            b.onclick = () => {
                applyPreset(name);
                this._syncPresets();
                this._syncWidgets();
