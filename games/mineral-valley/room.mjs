// 두 사람을 잇는 방. PeerJS 로 브라우저끼리 직접 연결한다.
const PREFIX='mineral-valley-v1-',ALPHABET='ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
...
export class ValleyRoom{
 constructor(hooks={},options={}){this.hooks=hooks;this.options=options;this.generation=0;this.role='';this.code='';this.status='offline';this.peer=null;this.connection=null;this.ready=false;this.lastSeen=0;this.started=0;}
 ...
 attach(connection){
  ...
  connection.on('open',()=>{if(this.guest)this.send({type:'hello'});});
  connection.on('data',message=>{
   ...
   if(message.type==='hello'&&this.host&&!this.ready){this.ready=true;clearTimeout(reservation);this.hooks.join?.();this.statusChanged('connected');this.send({type:'welcome'});return;}
 ...
 dropped(){
  const connection=this.connection;this.connection=null;this.ready=false;connection?.close();this.hooks.depart?.();
  if(this.host)this.statusChanged('waiting','친구가 나갔어요. 새 친구가 같은 코드로 들어올 수 있어요.');
 ...
 leave(){
  ...
  const wasActive=this.active;this.role='';this.code='';
