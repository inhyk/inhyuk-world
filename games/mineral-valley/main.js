import './style.css';
import { ORES,rankIndex,balance,redeem,GIFT_AMOUNT,LIMITS,MAX_CAPACITY,capacity,weightTotal,initial,power,speed,cost,pick,makeOre,orePrice,radius,canLift,buy,sell,reward,restore,money,rareBoost,RARE_DISTANCE,GIANT_CHANCE,GIANT_PRICE,balloonOre,MAP_SIZE,MAP_HALF,PLAY_LIMIT,clampMap } from './core.mjs';
import { BASE_ORE_COUNT,EVENT_ORE_COUNT,EVENT_CHANCES,EVENT_DURATION,eventStatus,pickEvent,timeLabel,dayPhase,AD_INTERVAL,AD_TEXT,MONSTER_MAX,MONSTER_HP,MONSTER_REWARD,BALLOON_RESPAWN } from './events.mjs';
import { MONSTER_REACH,MONSTER_BUMP,SWING_COOLDOWN,BALLOON_RANGE,inSwing,chase,knockback,ringPoint,monsterReward } from './life.mjs';
...
const makeActor=(index,tag,z)=>({index,tag,x:index*9,z,facing:0,carried:[],nearest:null,dist:Infinity,moving:false,swing:0});
const P1=makeActor(0,'1P',20),P2=makeActor(1,'2P',20);
let twoPlayer=false,net=null;
const crew=()=>twoPlayer||net?.partnered?[P1,P2]:[P1];
const held=o=>P1.carried.includes(o)||P2.carried.includes(o);
const ores=[],keys=new Set();let uid=0,clock=0,last=performance.now(),refresh=0,sweep=0,sound=false,toastUntil=8;
const monsters=[],music=createMusic();let balloon=null,balloonAt=0;
// 광물은 플레이어 주변 900m에만 깔리고, 1,500m 밖으로 밀려나면 회수해 다시 앞쪽에 심는다.
const FIELD_RADIUS=900,FIELD_KEEP=1500,MAP_VIEW=1400;
function save(){if(net?.guest)return;try{localStorage.setItem(KEY,JSON.stringify(state));$('save').textContent='진행 상황 자동 저장 ✓';}catch{$('save').textContent='저장 불가 · 이 창에서 계속 플레이 가능';}}
function toast(text){$('toast').textContent=text;$('toast').style.opacity=1;toastUntil=clock+5;}
function tone(high=false){blip(high?'sell':'pick');}
function blip(kind){if(!sound)return;try{music.blip(kind);}catch{}}
...
function syncEvent(){
 ...
 if(net?.guest){/* 손님은 방장이 보내 준 이벤트를 보기만 한다 */}
 else if(status.active&&activeEventCycle!==status.cycle){
...
let adSlot=Math.floor(state.playSeconds/AD_INTERVAL);
function syncAd(){
 const slot=Math.floor(state.playSeconds/AD_INTERVAL);
 if(slot>adSlot){adSlot=slot;toast(AD_TEXT);}
}
function syncDay(){
 ...
 if(sound)try{music.setPhase(phase.key);}catch{}
 return phase;
}
// 광물을 회수해 다시 심어 30,000m 맵 어디서나 밀도가 유지된다.
// 2인 플레이에서는 광물 밭을 사람 수대로 나눠, 혼자 멀리 떨어져도 주변이 비지 않게 한다.
function relocate(o,center){
 const index=ores.indexOf(o);
 if(index<0)return false;
 ores.splice(index,1);o.mesh?.dispose();
 fieldSpawn(center,o.eventOre);
 return true;
}
function recycle(limit=140){
 const team=crew(),share=Math.ceil(BASE_ORE_COUNT/team.length);
 const near=team.map(()=>[]),orphans=[];
 for(const o of ores){
  if(o.eventOre||held(o))continue;
  let owner=0,best=Infinity;
  for(let i=0;i<team.length;i++){const d=Math.hypot(o.x-team[i].x,o.z-team[i].z);if(d<best){best=d;owner=i;}}
  if(best>FIELD_KEEP)orphans.push(o);else near[owner].push({o,d:best});
 }
 let moved=0;
 const poorest=()=>{let i=0;for(let k=1;k<team.length;k++)if(near[k].length<near[i].length)i=k;return i;};
 // 1) 아무에게도 닿지 않는 광물은 가장 허전한 사람 앞에 다시 심는다.
 for(const o of orphans){
  if(moved>=limit)break;
  const i=poorest();
  if(relocate(o,team[i])){near[i].push({o:null,d:-1});moved++;}
 }
 // 2) 몫이 모자란 사람에게는 넉넉한 사람의 가장 먼 광물을 넘긴다.
 for(let i=0;i<team.length&&moved<limit;i++){
  while(near[i].length<share&&moved<limit){
   let rich=-1;
   for(let k=0;k<team.length;k++)if(k!==i&&near[k].length>share&&(rich<0||near[k].length>near[rich].length))rich=k;
   if(rich<0)break;
   near[rich].sort((a,b)=>b.d-a.d);
   const {o}=near[rich].shift();
   if(!o||!relocate(o,team[i]))continue;
   near[i].push({o:null,d:-1});moved++;
  }
 }
 return moved;
}
// 밤에만 나오는 몬스터. 아침이 오면 모두 사라진다.
function clearMonsters(){for(const m of monsters)m.mesh?.dispose();monsters.length=0;}
function spawnMonster(){
 const team=crew(),host=team[Math.floor(Math.random()*team.length)];
 const spot=ringPoint(host,90,340,Math.random,clampMap,150);
 const m={...spot,facing:0,hp:MONSTER_HP,cool:1.2,flash:0,mesh:null};
 monsters.push(m);return m;
}
function tickMonsters(dt,phase){
 if(net?.guest)return;
 if(phase.key!=='night'){if(monsters.length){clearMonsters();toast('아침 햇살에 몬스터가 모두 사라졌어요.');}return;}
 const team=crew();
 if(monsters.length<MONSTER_MAX&&Math.random()<dt*.35)spawnMonster();
 for(let i=monsters.length-1;i>=0;i--){
  const m=monsters[i];
  m.cool=Math.max(0,m.cool-dt);m.flash=Math.max(0,m.flash-dt);
  const seen=chase(m,team,dt);
  if(!seen){monsters.splice(i,1);m.mesh?.dispose();continue;}
  if(seen.distance<=MONSTER_REACH&&!m.cool){
   m.cool=2.8;knockback(seen.target,m,MONSTER_BUMP,clampMap);
   if(seen.target.carried.length){drop(seen.target,seen.target.carried.length-1);toast(`${seen.target.tag}: 몬스터에게 부딪혀 광석을 놓쳤어요! ${seen.target.index?'K':'T'} 키로 반격하세요.`);}
   else toast(`${seen.target.tag}: 몬스터가 달려들어요! ${seen.target.index?'K':'T'} 키로 쫓아내세요.`);
   blip('hit');
  }
  const range=Math.min(...team.map(a=>Math.hypot(m.x-a.x,m.z-a.z)));
  if(range>800){m.mesh?.dispose();monsters.splice(i,1);continue;}
  if(!m.mesh&&range<230)m.mesh=world.addMonster();
  else if(m.mesh&&range>320){m.mesh.dispose();m.mesh=null;}
  if(m.mesh){m.mesh.position.set(m.x,Math.sin(clock*3+i)*.4,m.z);m.mesh.rotation.y=m.facing;m.mesh.scaling.setAll(m.flash>0?1.3:1);}
 }
}
// 계곡 어딘가에 딱 하나 떠 있는 풍선.
function spawnBalloon(){
 const a=Math.random()*Math.PI*2,d=700+Math.random()*(PLAY_LIMIT-900);
 balloon={x:clampMap(Math.cos(a)*d),z:clampMap(Math.sin(a)*d)};
 return balloon;
}
function popBalloon(actor){
 if(!balloon)return false;
 if(actor.carried.length>=capacity(state)){toast(`${actor.tag}: 운반 칸이 가득 차서 풍선 선물을 받을 수 없어요.`);return true;}
 const o=spawn(balloonOre(),actor.x,actor.z);
 o.fromBalloon=true;o.mesh=world.addOre(o);
 actor.carried.push(o);state.found[o.id]++;
 balloon=null;balloonAt=state.playSeconds+BALLOON_RESPAWN;
 blip('pop');
 toast(`${actor.tag}: 풍선을 터뜨렸어요! ${oreName(o)} · ${money(orePrice(o))} — 무게와 상관없이 그대로 가져갑니다.`);
 save();update();targetUI();
 return true;
}
function tickBalloon(){
 if(!net?.guest&&!balloon&&state.playSeconds>=balloonAt)spawnBalloon();
 world.setBalloon(balloon,clock);
}
// 곡괭이 휘두르기. 풍선이 사정거리에 있으면 먼저 터뜨린다.
function swing(actor){
 if(net?.guest&&actor===P1)return void net.act('swing');
 if($('modal').open||actor.swing>0)return;
 actor.swing=1;
 if(balloon&&Math.hypot(balloon.x-actor.x,balloon.z-actor.z)<=BALLOON_RANGE&&popBalloon(actor))return;
 let hits=0,slain=0,earned=0;
 for(let i=monsters.length-1;i>=0;i--){
  const m=monsters[i];
  if(!inSwing(actor,m))continue;
  hits++;m.hp--;m.flash=.25;knockback(m,actor,8,clampMap);
  if(m.hp<=0){
   earned+=reward(state,monsterReward(MONSTER_REWARD,rareBoost(Math.hypot(m.x,m.z))));
   m.mesh?.dispose();monsters.splice(i,1);slain++;
  }
 }
 if(slain){toast(`${actor.tag}: 몬스터 ${slain}마리 처치! +${money(earned)}`);blip('slay');save();update();}
 else if(hits)blip('hit');
 else blip('pick');
}
function update(){
...
function targetUI(){
 ...
 if(!o){
  ...
  $('target-info').textContent=P1.carried.length?'R 귀환 · F 모두 판매 · 광석 칸을 눌러 내려놓기':'WASD 이동 · 드래그 시점 · T 공격 · 2 키 분할 2인 · 온라인 버튼으로 친구와';
...
function lift(actor){
 if(net?.guest&&actor===P1)return void net.act('lift');
...
function drop(actor,index=actor.carried.length-1){
 if(net?.guest&&actor===P1)return void net.act('drop',index);
...
function home(actor){if(net?.guest&&actor===P1)return void net.act('home');actor.x=actor.index*9;actor.z=13;toast(`${actor.tag}: 베이스캠프 도착! 판매소와 상점을 이용하세요.`);}
...
function sale(actor){
 if(net?.guest&&actor===P1)return void net.act('sell');
...
function setTwoPlayer(on){
 if(on&&net?.partnered){toast('온라인으로 친구와 연결된 동안에는 분할 화면을 쓸 수 없어요.');return;}
...
function resetPrompt(){
 ...
 $('confirm-reset').onclick=()=>{
  ...
  clearMonsters();balloon=null;balloonAt=0;adSlot=0;
  Object.assign(state,initial());activeEventCycle=0;populate();spawnBalloon();
...
$('sound').onclick=()=>{sound=!sound;$('sound').textContent=`소리 ${sound?'ON':'OFF'}`;$('sound').setAttribute('aria-label',sound?'소리 끄기':'소리 켜기');try{if(sound){music.setPhase(dayPhase(state.playSeconds).key);music.start();}else music.stop();}catch{}tone();};
$('modal-body').onsubmit=e=>{if(e.target.id!=='gift-form')return;e.preventDefault();if(net?.guest){net.act('code',$('gift-code').value);$('gift-result').textContent='방장에게 코드를 보냈어요.';return;}if(redeem(state,$('gift-code').value)){save();update();tone(true);shop();$('gift-result').textContent=`${money(GIFT_AMOUNT)} 지급 완료!`;}else $('gift-result').textContent='코드가 맞지 않아요. 다시 확인해 주세요.';};
$('modal-body').onclick=e=>{const b=e.target.closest('[data-buy]');if(!b)return;if(net?.guest){net.act('buy',b.dataset.buy);tone(true);return;}if(buy(state,b.dataset.buy)){tone(true);save();update();shop();}};
// 1인 플레이에서는 방향키가 WASD를 대신하고, 2인 플레이에서는 2P 전용이 된다.
const ARROWS={w:'arrowup',s:'arrowdown',a:'arrowleft',d:'arrowright'};
const HOTKEYS=new Set(['w','a','s','d','e','q','r','f','b','t','2',' ','arrowup','arrowdown','arrowleft','arrowright','.',',','/',"'",'l',';','k']);
addEventListener('keydown',e=>{
 ...
 keys.add(k);
 if(e.repeat)return;
 if(k==='e')lift(P1);if(k==='q')drop(P1);if(k==='r')home(P1);if(k==='f')sale(P1);if(k==='b')shop();if(k==='t')swing(P1);
 if(k==='2')setTwoPlayer(!twoPlayer);
 if(twoPlayer){if(k==='.')lift(P2);if(k===',')drop(P2);if(k==='/')sale(P2);if(k==="'")home(P2);if(k==='k')swing(P2);}
});
...
function applyMove(actor,axis,dt){
 actor.moving=false;
 let dx=axis.x??axis.mx??0,dz=axis.z??axis.mz??0;
 if(!dx&&!dz)return;
 const n=Math.hypot(dx,dz);dx/=n;dz/=n;
 const yaw=Number.isFinite(axis.yaw)?axis.yaw:world.yaw(actor.index);
 const vx=dx*Math.cos(yaw)-dz*Math.sin(yaw),vz=dx*Math.sin(yaw)+dz*Math.cos(yaw);
 const pace=speed(state)*(actor.carried.length?Math.max(.3,1-.3*weightTotal(actor.carried)/power(state)):1);
 actor.x=clampMap(actor.x+vx*pace*dt);actor.z=clampMap(actor.z+vz*pace*dt);
 actor.facing=Math.atan2(vx,vz);actor.moving=true;
}
...
function map(){
 ...
 // 밤에도 지도는 하얗게 — 어두운 화면에서 길을 잃지 않도록.
 ctx.fillStyle='#e9eee0';ctx.fillRect(0,0,180,180);
 ctx.strokeStyle='#cbd5c0';ctx.lineWidth=1;
 ...
 const[ox,oz]=at(0,0);
 ctx.strokeStyle='#9bb58c';ctx.setLineDash([3,3]);ctx.beginPath();ctx.arc(ox,oz,RARE_DISTANCE*s,0,6.3);ctx.stroke();ctx.setLineDash([]);
 ctx.strokeStyle='#93a58b';ctx.lineWidth=2;const[bx,bz]=at(-MAP_HALF,MAP_HALF);ctx.strokeRect(bx,bz,MAP_SIZE*s,MAP_SIZE*s);
 for(const o of ores){
  if(held(o))continue;
 ...
 // 밤 몬스터는 붉은 점으로
 ctx.fillStyle='#e04a63';
 for(const m of monsters){const[px,py]=at(m.x,m.z);if(px<0||px>180||py<0||py>180)continue;ctx.beginPath();ctx.arc(px,py,2.6,0,6.3);ctx.fill();}
 // 풍선은 화면 밖이면 가장자리 화살표로 방향을 알려 준다
 if(balloon){
  const[bx,by]=at(balloon.x,balloon.z);
  const inside=bx>4&&bx<176&&by>4&&by<176;
  const a=Math.atan2(by-90,bx-90);
  const px=inside?bx:90+Math.cos(a)*80,py=inside?by:90+Math.sin(a)*80;
  ctx.fillStyle='#ff5fa2';ctx.strokeStyle='#fffdf0';ctx.lineWidth=1.5;
  ctx.beginPath();ctx.arc(px,py,inside?5:4.5,0,6.3);ctx.fill();ctx.stroke();
 }
 for(const a of team){
 ...
 $('balloon-hint').textContent=balloon?`🎈 풍선 ${COMPASS[Math.round(Math.atan2(balloon.x-P1.x,balloon.z-P1.z)/(Math.PI/4)+8)%8]} ${Math.round(Math.hypot(balloon.x-P1.x,balloon.z-P1.z)).toLocaleString()}m`:`🎈 새 풍선 준비 중 · ${timeLabel(balloonAt-state.playSeconds)}`;
 $('monster-hint').textContent=monsters.length?`👾 몬스터 ${monsters.length}마리 · T(1P)·K(2P)로 공격`:'';
 $('monster-hint').hidden=!monsters.length;
}
const COMPASS=['↑','↗','→','↘','↓','↙','←','↖'];
...
function frame(now){
 ...
 if(!document.hidden&&!net?.guest)state.playSeconds+=elapsed;
 clock+=dt;
 const team=crew();
 // 2P는 L·; 키로 직접 시점을 돌린다.
 if(twoPlayer&&!$('modal').open){
  if(keys.has('l'))world.setYaw(-1.9*dt,1);
  if(keys.has(';'))world.setYaw(1.9*dt,1);
 }
 if(!$('modal').open)for(const a of team){
  // 온라인에서는 친구 캐릭터를 네트워크가 움직인다.
  if(a===P2&&net?.partnered)continue;
  applyMove(a,moveInput(a),dt);
 ...
 net?.tick(dt);
 for(const a of team)a.swing=Math.max(0,a.swing-dt/SWING_COOLDOWN);
 tickMonsters(dt,dayPhase(state.playSeconds));
 tickBalloon();
 ...
 refresh+=dt;
 if(refresh>.15){
  refresh=0;syncEvent();syncDay();syncAd();targetUI();map();world.streamScenery(team);
  $('region').textContent=regionName(Math.hypot(P1.x,P1.z));
 }
 sweep+=dt;
 if(sweep>1){sweep=0;if(!document.hidden&&!net?.guest)recycle(crew().length>1?460:160);}
 requestAnimationFrame(frame);
}
setInterval(save,5000);addEventListener('pagehide',save);
addEventListener('resize',()=>world.resize());
net=setupOnline({
 state,me:P1,partner:P2,ores,monsters,world,
 toast,clearInput,applyMove,
 moveAxis:()=>moveInput(P1),
 yaw:()=>world.yaw(0),
 balloon:()=>balloon,
 setBalloon(v){balloon=v;},
 setPartnered(on){
  document.body.classList.toggle('online',on);
  if(on&&twoPlayer)setTwoPlayer(false);
  world.showPartner(on||twoPlayer);
  if(!on&&!twoPlayer){P2.carried=[];P2.nearest=null;}
  recycle(4000);update();targetUI();
 },
 clearField(){
  for(const o of ores)o.mesh?.dispose();ores.length=0;
  P1.carried=[];P2.carried=[];P1.nearest=null;P2.nearest=null;
  for(const m of monsters)m.mesh?.dispose();monsters.length=0;
  balloon=null;world.setBalloon(null,0);
  update();targetUI();
 },
 dropAll(actor){for(const o of actor.carried){const i=ores.indexOf(o);if(i>=0)ores.splice(i,1);o.mesh?.dispose();}actor.carried=[];},
 rebuild(){
  for(const o of ores)o.mesh?.dispose();ores.length=0;
  P1.carried=[];P2.carried=[];P1.nearest=null;P2.nearest=null;
  populate();spawnBalloon();update();targetUI();
 },
 refresh(){update();targetUI();},
 act:{
  lift,drop,home,swing,
  sale(actor){if(!atCamp(actor)){toast(`${actor.tag}: 판매소는 캠프에 있어요.`);return;}doSell(actor);},
  buy(key){if(buy(state,key)){save();update();}},
  redeem(code){if(redeem(state,code)){save();update();}},
 },
});
spawnBalloon();update();syncEvent();syncDay();map();targetUI();requestAnimationFrame(frame);
window.render_game_to_text=()=>JSON.stringify({
 coordinates:`x,z: -${MAP_HALF}..${MAP_HALF}; y up; map ${MAP_SIZE}×${MAP_SIZE}`,
 player:{x:P1.x,z:P1.z,facing:P1.facing},twoPlayer,partnered:!!(twoPlayer||net?.partnered),
 player2:twoPlayer||net?.partnered?{x:P2.x,z:P2.z,facing:P2.facing}:null,
 ...
 monsters:monsters.map(m=>({x:Math.round(m.x),z:Math.round(m.z),hp:m.hp})),
 balloon:balloon?{x:Math.round(balloon.x),z:Math.round(balloon.z),distance:Math.round(Math.hypot(balloon.x-P1.x,balloon.z-P1.z))}:null,
 music:music.playing,sound,online:net?net.state():null,
...
if(import.meta.env.DEV&&new URLSearchParams(location.search).has('test'))window.__mineralTest={state,player:P1,player2:P2,ores,spawn,update,advanceEvents,setTwoPlayer,recycle,P1,P2,orePrice,scene:world.scene,get net(){return net;},applyMove,capacity:()=>capacity(state),monsters,spawnMonster,clearMonsters,swing,get balloon(){return balloon;},spawnBalloon,popBalloon,music,lift:(a=P1)=>lift(a),sale:(a=P1)=>sale(a),drop:(a=P1,i)=>drop(a,i??a.carried.length-1),home:(a=P1)=>home(a)};
